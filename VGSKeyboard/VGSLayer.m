...
static unsigned short imgbuf[2][VRAM_WIDTH * VRAM_HEIGHT * 2];
...
- (id)init
{
    if (self = [super init]) {
        ...
        for (int i = 0; i < 2; i++) {
            img[i] = CGBitmapContextCreate(imgbuf[i], VRAM_WIDTH, VRAM_HEIGHT, 5, 2 * VRAM_WIDTH, colorSpace, kCGImageAlphaNoneSkipFirst | kCGBitmapByteOrder16Little);
        }
